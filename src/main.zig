...
const xr = @import("openxr"); // generated by openxr-zig; we just ensure it compiles
...
/// Minimal OpenXR “demo”: we don’t touch any fields of `xr`
/// because the generated API surface varies by commit.
/// We only prove that:
///   - build.zig ran the generator successfully
///   - the `openxr` module can be imported and used as a value.
fn logOpenXRBindingInfo() void {
    // Use `xr` as a value so Zig doesn’t complain that it’s unused.
    _ = xr;
    std.debug.print("OpenXR bindings imported (openxr-zig generator succeeded).\n", .{});
}
...
pub fn main() !void {
    ...
    // Just after GLFW is alive, prove that openxr-zig is wired up.
    logOpenXRBindingInfo();
